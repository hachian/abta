...
- 記事のフロントマターで `draft: true` を設定
- **開発環境（npm run dev）**: 下書き記事も表示
- **本番環境（npm run build）**: 下書き記事は非公開
- 執筆中の記事や非公開にしたい記事の管理に最適
...
- **Critical CSS分離**: Above the fold専用CSS（91行）で初期表示高速化
- **Non-critical CSS遅延読み込み**: Below the fold CSS（885行）を後読み
- **完全CSSインライン化**: レンダリングブロッキング解消
- **Lighthouse 100点達成**: FCP/LCP大幅改善
