...
- **共通コンポーネント**:
  ...
  - `Lightbox.astro` - 画像クリック時の拡大表示（ライトボックス）
- TypeScript厳密モードが有効
...
ブログ記事内の画像をクリックすると拡大表示される機能を実装しています。

### 機能詳細

- **対象画像**:
  - ヒーロー画像（記事トップの画像）
  - ブログ記事内のすべての画像（リンクでラップされていないもの）
- **操作方法**:
  - 画像クリックで拡大表示
...
- **アクセシビリティ**:
  ...
  - フォーカス管理とフォーカストラップ
- **実装ファイル**:
  - `src/components/Lightbox.astro` - ライトボックスコンポーネント
  - `src/styles/components.css` - ライトボックス用スタイル（.lightbox関連）